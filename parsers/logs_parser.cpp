...
std::string ip_to_str(be32_t ip) {
    std::string ip_str;
    ip_str.clear();
    uint8_t ip_fields[4];
    for ( int i = 0; i < 4; i++ ) {
        ip_str.append(std::to_string(ip_fields[i]) + '.');
    }
    /* erase '.' at the end */
    ip_str.erase(ip_str.size() - 1);

    return ip_str;
}

using field_pairings_t = std::vector<std::pair<const char *, int>>;
static field_pairings_t proto_converter = {{"tcp", TCP}, {"udp", UDP}, {"icmp", ICMP}};
static field_pairings_t action_converter = {{"accept", PKT_PASS}, {"drop", PKT_DROP}};
...
std::string fmt_log(log_row_t log) {
    ...
    append_field(log_txt, ip_to_str(log.saddr));
    append_field(log_txt, ip_to_str(log.daddr));
